...
import { connectDb, closeDb } from '../db/store.js'
...
app.use(graceful.trackRequests)
...
export async function runServer() {
  const [sessionMiddleware] = await Promise.all([
    createSessionMiddleware(),
    connectDb(),
  ])

  app.get('/api/health', graceful.livenessHandler)
  app.get('/api/ready', graceful.readinessHandler)
  ...
  await new Promise((resolve, reject) => {
    ...
    graceful.attachServer(httpServer)
  })
